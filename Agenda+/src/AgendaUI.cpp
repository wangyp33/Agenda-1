...
void AgendaUI::userLogIn(void) {
    ...
    if (m_agendaService.userLogIn(t_username, t_password)) {
        ...
        cout << "[error] Password error or user doesn't exist!\n[error] Log in failed!\n\n";
};
...
void AgendaUI::userRegister(void) {
    cout << "\n"
         << "[register] [username] [password] [email(xxxx@xx.xxx)] [phone(020-88... or 138... or +86135...)]\n"
         << "[register] ";
    std::string t_username, t_password, t_email, t_phone;
    cin >> t_username >> t_password >> t_email >> t_phone;
    //regular expression test for t_email's format
    const std::regex pattern_email("(\\w+)(\\.|_)?(\\w*)@(\\w+)(\\.(\\w+))+");
    bool is_email_valid = std::regex_match(t_email, pattern_email);
    if (!is_email_valid)
        cout << "[error] The format of email is invalid!\n";
    //regular expression test for t_phone's format
    const std::regex pattern_phone1("(\\d+)");
    const std::regex pattern_phone2("((\\d+(\\-))+)(\\d+)");
    const std::regex pattern_phone3("(\\+)?(\\d+)");
    bool is_phone_valid = std::regex_match(t_phone, pattern_phone1) 
                       || std::regex_match(t_phone, pattern_phone2)
                       || std::regex_match(t_phone, pattern_phone3);
    if (!is_phone_valid)
        cout << "[error] The format of phone is invalid!\n";
    //if one of them invalid, return
    if (!is_email_valid || !is_phone_valid) {
        cout << "[error] Regiser failed!\n\n";
        return;
    }
    //register a user
    if (m_agendaService.userRegister(t_username, t_password, t_email, t_phone))
        ...
        cout << "[error] This username has been registered!\n[error] Regiser failed!\n\n";
};
...
void AgendaUI::createMeeting(void) {
    ...
    time_enter: cout << "[create meeting] [start time(yyyy-mm-dd/hh:mm)] [end time(yyyy-mm-dd/hh:mm)]\n"
         << "[create meeting] ";
    cin >> t_stime >> t_etime;
    if (!Date::isValid(Date::stringToDate(t_stime)) || !Date::isValid(Date::stringToDate(t_etime)) || Date::stringToDate(t_stime) >= Date::stringToDate(t_etime)) {
        cout << "[error] Date invalid! Please try again!\n";
        goto time_enter;
    }
...
void AgendaUI::addMeetingParticipator(void) {
    ...
    cin.sync();
    getline(cin, t_title);
    cout << "[add participator] [participator username]\n"
    	 << "[add participator] ";
    cin >> t_par_name;
...
void AgendaUI::removeMeetingParticipator(void) {
    string t_title, t_par_name;
    cout << "\n"
         << "[remove participator] [meeting title]\n"
         << "[remove participator] ";
    cin.clear();
    cin.sync();
    getline(cin, t_title);
    cout << "[remove participator] [participator username]\n"
    	 << "[remove participator] ";
    cin >> t_par_name;
...
void AgendaUI::quitMeeting(void) {
    ...
    if (m_agendaService.quitMeeting(m_userName, t_title))
        ...
        cout << "[error] There is no such a meeting or you are not the participator of it!\n";
...
void AgendaUI::queryMeetingByTimeInterval(void) {
    ...
    cout << "\n";   
    t_time_enter: cout << "[query meetings] [start time(yyyy-mm-dd/hh:mm)] [end time(yyyy-mm-dd/hh:mm)]\n"
         << "[query meetings] ";
    cin >> t_stime >> t_etime;
    if (!Date::isValid(Date::stringToDate(t_stime)) || !Date::isValid(Date::stringToDate(t_etime)) || Date::stringToDate(t_stime) > Date::stringToDate(t_etime)) {
            cout << "[error] Time format or interval error! Please try again!\n";
            goto t_time_enter;
    }
...
void AgendaUI::deleteMeetingByTitle(void) {
    ...
    if (m_agendaService.deleteMeeting(m_userName, t_title))
        ...
        cout << "[error] There is no such a meeting or you are not the sponsor!\n";
