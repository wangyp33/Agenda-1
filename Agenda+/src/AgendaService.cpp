...
bool AgendaService::createMeeting(const std::string &userName, const std::string &title,
    ...
    if (participator.size() == 0) {
        cout << "[error] The number of participators is 0!\n";
        return false;
    }
    ...
    if((m_storage->queryMeeting(t_filter)).size() > 0) {
        cout << "[error] This meeting has been created! Please check the title!\n";
        return false;
    }
    //test the Date
    ...
    for (auto it : my_meeting_list) {
        if (  !(it.getEndDate() <= s_date || it.getStartDate() >= e_date)  ) {
            cout << "[error] There is time conflict of the sponsor!\n";
            return false;
        }
    }
    //create meeting
    ...
    for (auto &it : participator) {
        ...
        if (_isParticipator == false) {
            ...
            cout << "[error] Adding participators error!\n";
            return false;
        }
    }
    return true;
...
bool AgendaService::addMeetingParticipator(const std::string &userName,
    ...
    //test if participator has registered
    ...
    if (!is_par_regist) {
        cout << "[error] The participator \"" << participator << "\" has not registered!\n";
        return false;
    }
    ...
    if (t_meeting_list.size() == 0) {
        cout << "[error] There is no such a meeting or you are not its sponsor! Please check the title of the meeting!\n";
        return false;
    }
    //get the meeting
    ...
    for (auto it : participator_meeting_list) {
        if (  !(t_meeting.getEndDate() <= it.getStartDate() || t_meeting.getStartDate() >= it.getEndDate())  ) {
            cout << "[error] There is time conflict of the participator \"" << participator << "\"!\n";
            return false;
        }
    }
